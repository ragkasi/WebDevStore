<template>
  <div>
    ...
    <b-alert variant="danger" v-model="showFailure" dismissible>
      Error editing part! <b>{{ failureReason }}</b> Try again later.
    </b-alert>
    <h2>Edit Part</h2>
    <hr />
    ...
    <b-form @submit.prevent v-if="loaded">
      <b-form-group
        id="input-part-name"
        label="Item Name:"
      ...
      >
        <label v-if="oldFile"><b>Current file:</b> {{ oldFile }}</label>
        <b-form-file
          id="input-files"
          v-model="form.file"
          accept=".zip"
      ...
      <br />
      <b-button @click="onSubmit" variant="danger">
        Submit part for editing
      </b-button>
    </b-form>
  </div>
</template>
...
export default class PartsEdit extends Vue {
  ...
  mounted() {
    ...
    parts.getPart(this.id).then((res) => {
      if (res.data?.status === 'success') {
        ...
        this.form = {
          ...
          canPurchase: part.canPurchase,
          canReturn: part.canReturn,
          file: null,
          previewFile: null,
        };
        this.oldPreviewImage = part.previewImage?.filename ?? null;
        this.oldFile = part.file?.filename ?? null;
        this.loaded = true;
      } else {
        this.$router.push('/parts/edit').catch(() => {});
      }
    });
  }
