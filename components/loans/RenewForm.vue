<template>
  <div>
    <b-alert variant="danger" v-model="showFailure" dismissible>
      Error renewing tools! <b>{{ failureReason }}</b> Try again later.
    </b-alert>
    ...
    <b-form @submit="onSubmit">
      ...
      <b-form-group
        id="input-group-renewed-items"
        label-for="input-renewed-items"
      >
        <multiselect
          id="input-renewed-items"
          v-model="renewedItems"
          :options="options"
          :multiple="true"
          :searchable="false"
          ...
          label="label"
          track-by="value"
          placeholder="Pick a item to renew"
        ></multiselect>
      </b-form-group>
      <b-row>
        <b-col style="m-4">
        ...
        <b-col>
          <div style="float: right">
            <b-btn
              type="submit"
              variant="danger"
              :disabled="renewedItems.length == 0 || disableSubmit"
            >
              Renew Loans
            </b-btn>
          </div>
...
@Component({})
export default class RenewForm extends Vue {
  @Prop(Array) tools!: Tool[];

  renewedItems: { label: string; value: string }[] = [];
  ...
  onSubmit(evt: Event) {
    ...
    let itemIds: string[] = [];
    this.renewedItems.forEach((elem) => {
      itemIds.push(elem.value as string);
    });
    api.renewLoans(itemIds).then((res) => {
      if (res.data?.status == 'success') {
